(ns physiotherapy-support.actor
  "PhysiotherapySupportActor — the ISCO-08 3255 independent
  physiotherapy-support actor as a `langgraph.graph/state-graph` (per
  ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  support operation request (intake → advise → govern → decide →
  ...
  The unconditional invariant: the PhysiotherapySupportAdvisor can
  never directly commit a record or dispatch a robot action the
  PhysiotherapySupportGovernor refuses — every commit-record! call is
  gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [physiotherapy-support.advisor :as advisor]
            [physiotherapy-support.governor :as governor]
            [physiotherapy-support.store :as store]))

(defn build-graph
  "Build a compiled PhysiotherapySupportActor graph. `store`
  implements `physiotherapy-support.store/Store`. `advisor` implements
  `physiotherapy-support.advisor/Advisor` (defaults to `mock-advisor`).
